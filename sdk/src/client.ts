...
export class SagatClient {
  ...
  constructor(
    ...
    this.#apiUrl = apiUrl;
    this.#mode = mode;
    this.#fetchOverride = fetchOverride || ((url, init) => fetch(url, init));
  }

  async connect(signature: string, expiry: string) {
    const response = await this.#request<AuthResponse>('/auth/connect', {
      method: 'POST',
  ...
  async rejectMultisigInvite(
    ...
    return this.#request<{ message: string; address: string }>(
  ...
  async getProposals(
    ...
    if (params.nextCursor) {
      searchParams.append('nextCursor', params.nextCursor.toString());
    }
    if (params.perPage) {
    ...
    return this.#request<PaginatedResponse<ProposalWithSignatures>>(
  ...
  > {
    return this.#request<Record<string, MultisigWithMembers[]>>(
  ...
  async #request<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
    const response = await this.#fetchOverride(`${this.#apiUrl}${endpoint}`, {
      ...
      credentials: this.#mode === 'cookie' ? 'include' : undefined,
    });
